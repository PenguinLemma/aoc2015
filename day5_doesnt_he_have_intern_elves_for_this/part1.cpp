...
{
    return ((a == 'a') and (b == 'b')) or ((a == 'c') and (b == 'd')) or
           ((a == 'p') and (b == 'q')) or ((a == 'x') and (b == 'y'));
}

bool IsNice(const std::string& s)
{
    // check that there are 3 or more vowels
    int num_vowels = std::count_if(std::begin(s), std::end(s), [](char c) { return IsVowel(c); });
    if (num_vowels < 3)
        return false;  // but I'm sure you are nice according to some other criteria

    // check that contains at least one doubled letter
    auto adj_equal_letters =
        std::adjacent_find(std::begin(s), std::end(s), [](char a, char b) { return a == b; });
    ...
    auto forbidden_pair = std::adjacent_find(
        std::begin(s), std::end(s), [](char a, char b) { return IsForbidden(a, b); });
